...
class FakeParser:
    ...
    def parse_document(self, file_path, output_dir, method="auto", **kwargs):
        ...
        self.processed_kwargs.append(kwargs)
...
def test_incremental_reprocesses_when_parser_options_change(monkeypatch, tmp_path):
    batch_parser, fake_parser = _make_batch_parser(monkeypatch)
    docs_dir, first_doc, second_doc = _seed_two_docs(tmp_path)
    output_dir = tmp_path / "out"
...
def test_duplicate_stems_use_distinct_output_directories(monkeypatch, tmp_path):
    batch_parser, fake_parser = _make_batch_parser(monkeypatch)
    ...
    result = batch_parser.process_batch(
        [str(first_dir), str(second_dir)], str(tmp_path / "out")
    )

    assert len(result.successful_files) == 2
    assert len(set(fake_parser.processed_outputs)) == 2
    assert all(
        Path(path).name.startswith("report_") for path in fake_parser.processed_outputs
    )


def test_overlapping_inputs_are_deduplicated(monkeypatch, tmp_path):
    batch_parser, fake_parser = _make_batch_parser(monkeypatch)
    docs_dir = tmp_path / "docs"
    docs_dir.mkdir()
    document = docs_dir / "only.txt"
    document.write_text("content", encoding="utf-8")

    result = batch_parser.process_batch(
        [str(docs_dir), str(document)], str(tmp_path / "out")
    )

    assert result.total_files == 1
    assert fake_parser.processed_files == [str(document)]


def test_output_tree_is_excluded_from_recursive_input(monkeypatch, tmp_path):
    batch_parser, fake_parser = _make_batch_parser(monkeypatch)
    docs_dir = tmp_path / "docs"
    output_dir = docs_dir / "generated"
    output_dir.mkdir(parents=True)
    source = docs_dir / "source.txt"
    generated = output_dir / "previous.txt"
    source.write_text("source", encoding="utf-8")
    generated.write_text("generated", encoding="utf-8")

    result = batch_parser.process_batch([str(docs_dir)], str(output_dir))

    assert result.total_files == 1
    assert fake_parser.processed_files == [str(source)]


def test_timeout_is_per_file_not_for_whole_batch(monkeypatch, tmp_path):
    batch_parser, fake_parser = _make_batch_parser(monkeypatch)
    batch_parser.timeout_per_file = 1
    fake_parser.delay = 0.6
    docs_dir, first_doc, second_doc = _seed_two_docs(tmp_path)

    result = batch_parser.process_batch([str(docs_dir)], str(tmp_path / "out"))

    assert set(result.successful_files) == {str(first_doc), str(second_doc)}
    assert all(options["timeout"] == 1 for options in fake_parser.processed_kwargs)
